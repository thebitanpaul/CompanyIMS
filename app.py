...
# Function to insert data into the database from an Excel file
def insert_data_excel(conn, data):
    try:
        c = conn.cursor()
        for row in data:
            company_name = row[0]  # Assuming company name is the first column
            existing_data = search_data(conn, company_name)
            if existing_data:
                # If company data already exists, update it
                update_data(conn, *row)
                
            else:
                # If company data doesn't exist, insert it as new
                c.execute('''INSERT INTO companies 
                             (company_name, communication, legal_information, activity, presentation, general_information, remarks) 
                             VALUES (?, ?, ?, ?, ?, ?, ?)''', row)
        ...
        if uploaded_file is not None:
            try:
                ...
                if all(col in df.columns for col in expected_columns):
                    ...
                    st.sidebar.success("Excel data saved successfully!")
                else:
